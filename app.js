document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeSubTabs(cardId, subTabToSelect = null) {
        const cardData = battleCardData[cardId];
        if (!cardData || !cardData.tabs || cardData.tabs.length === 0) return;
    ...
    mainContent.addEventListener('dblclick', (e) => {
        ...
        if (editable) {
            ...
            const isMultiLine = path.includes('.details[') || path.includes('.subtitle') || path.includes('.description') || path.includes('.painPoint') || path.includes('.solution');

            const originalHTML = editable.innerHTML;
            const input = isHeader || !isMultiLine ? document.createElement('input') : document.createElement('textarea');
            if (!isMultiLine) input.type = 'text';

            input.value = getObjectByPath(battleCardData, path);
            input.className = "w-full h-auto p-1 border rounded bg-white";
            if (isMultiLine) input.style.minHeight = `${editable.offsetHeight + 20}px`;
            ...
            const saveChange = () => { 
                ...
                updateObjectByPath(battleCardData, path, newValue); 
                saveData(); 
